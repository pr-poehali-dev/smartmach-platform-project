...
Проведён аудит реализации на соответствие заявленному. Найдены и исправлены
пять расхождений — все закрыты автотестами.

**1. Диагностика кромки была недостижима в интерфейсе.** Дефект «шероховатость»
имел нулевой шаг коррекции, а генератор пропускал такие записи. Перебор всех
комбинаций настроек панели (720 сценариев) показал: кнопка «Применить
рекомендации по кромке» не активировалась **ни разу**. Шероховатости задано
реальное действие: до устранения причины снижается скорость, что уменьшает
шаг борозд. Кнопка активна в 48 из 72 проверенных сценариев.

**2. Оператор не мог задать свой режим.** Параметры отображались только для
чтения, поэтому проверить режим «как в цеху» и увидеть прогноз грата было
невозможно — все сценарии давали индекс 100. Параметры сделаны редактируемыми
ползунками в пределах технологических допусков.

**3. Смещение фокуса корректировалось процентом.** Параметр отрицательный,
поэтому относительный шаг давал бессмысленный результат: переход с −0,5
на −2,2 мм отображался как «−340 %». Введена установка абсолютного расчётного
значения; процент для таких коррекций не показывается.

**4. Экономия завышалась при совпадающих режимах.** Если оператор не менял
режим, разница держалась только на разной доле брака, но подавалась как
экономия от комплекса. Добавлен признак совпадения режимов и явная оговорка
в интерфейсе.

**5. Ряд толщин не совпадал между контурами.** Во фронтенде отсутствовал
процесс Ст3 8 мм, хотя серверная база и документация заявляли ряд 6/8/10/12 мм.
Процесс добавлен.

Согласованность контуров после правок перепроверена: TypeScript и Python дают
идентичный результат, включая новую логику фокуса.
